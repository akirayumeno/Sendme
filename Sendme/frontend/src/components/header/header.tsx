...
const Header = ({ messageCount, onThemeToggle, themeConfig }: {
  messageCount: number;
  onThemeToggle: () => void;
      ...
      <div className="flex items-center space-x-4">
        ...
        <button
          ...
          className={
            ...
            themeConfig.isDark ? 'bg-gray-700 hover:bg-gray-600' : 'bg-white hover:bg-gray-100'
          }`}
        >
          {themeConfig.isDark ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
        </button>
      </div>
    </div>
  );
